                ...
                \_ ->
                    let
                        ...
                        encoded =
                            CloudEvents.encodeCloudEvent event
                                |> Decode.decodeString
                                    (Decode.succeed (\c d e s t ext -> ( c, d, e, s, t, ext ))
                                        |> Pipeline.required "datacontenttype" Decode.string
                                        |> Pipeline.required "datacontentencoding" Decode.string
                                        |> Pipeline.required "subject" Decode.string
                                        |> Pipeline.required "time" Decode.string
                                        |> Pipeline.required "custom" Decode.string
                                    )
                    in
                    case encoded of
                        Ok ( contentType, encoding, subject, timeVal, custom ) ->
                            Expect.all
                                [ \_ -> Expect.equal "application/json" contentType
                                , \_ -> Expect.equal "base64" encoding
                                , \_ -> Expect.equal "test-subject" subject
                                , \_ -> Expect.equal "2024-01-01T00:00:00Z" timeVal
                                , \_ -> Expect.equal "value" custom
                                ]
                                ()
            ...
            [ test "new creates event with default values" <|
                \_ ->
                    ...
                    Expect.all
                        ...
                        , \_ -> Expect.equal Nothing event.time
                        , \_ -> Expect.true "extensions should be empty" (Dict.isEmpty event.extensions)
                        ]
                        ()
            ]
