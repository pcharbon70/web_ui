        ...
        , describe "init"
            [ test "4.5.1 - init creates initial state" <|
                \_ ->
                    ...
                    in
                    Expect.all
                        [ \_ -> Expect.true "Initial state should be Connecting" (model.state == WebSocket.Connecting)
                        , \_ -> Expect.true "Queue should be empty" (List.isEmpty model.queue)
                ...
                \_ ->
                    let
                        ...
                        ( newModel, _ ) =
                            WebSocket.send "test message" model config
                    in
                    Expect.true "Queue should remain empty when connected" (List.isEmpty newModel.queue)
            ]
        ...
        , describe "isConnected"
            [ test "Returns True when Connected" <|
                \_ ->
                    ...
                    in
                    Expect.isTrue (WebSocket.isConnected model)
            , test "Returns False when Disconnected" <|
                \_ ->
                    ...
                    in
                    Expect.isFalse (WebSocket.isConnected model)
