...
[ ] 1 Phase 1 - Transport Backbone and CloudEvent Boundary
  ...
  [ ] 1.1 Section - Endpoint and Router Bootstrap
    Implement deterministic endpoint and router boundaries for SPA delivery and websocket transport admission.

    [ ] 1.1.1 Task - Implement canonical HTTP and websocket route surfaces
      Define stable route boundaries and channel mount points required by transport contracts.

      [ ] 1.1.1.1 Subtask - Implement endpoint configuration for static Elm/Tailwind assets.
      [ ] 1.1.1.2 Subtask - Implement router entries for SPA shell and websocket channel handshake.
      [ ] 1.1.1.3 Subtask - Implement startup checks that fail closed on missing transport configuration.

    [ ] 1.1.2 Task - Implement deterministic websocket naming contracts
      Enforce canonical topic and event-name contracts from `service_contract.md`.

      [ ] 1.1.2.1 Subtask - Implement topic validation for `webui:runtime:v1` and session-scoped variants.
      [ ] 1.1.2.2 Subtask - Implement known event-name validation for send/ping and recv/error/pong flows.
      [ ] 1.1.2.3 Subtask - Implement typed protocol errors for unknown event names.
