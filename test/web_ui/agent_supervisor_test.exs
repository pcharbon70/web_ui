defmodule WebUI.AgentSupervisorTest do
  ...
  describe "5.2.1 - supervisor starts and manages agents" do
    setup do
      # Start the supervisor for testing
    ...
    test "start_agent/3 starts an unnamed agent" do
      ...
      assert {:ok, pid} = AgentSupervisor.start_agent(TestAgent1, [])
      assert is_pid(pid)
  ...
  describe "5.2.2 - registry tracks subscriptions" do
    ...
    test "registry tracks multiple agents per pattern" do
      ...
      agents = AgentRegistry.lookup("com.shared.event")
  ...
  describe "5.2.3 - lookup finds agents by event type" do
    ...
    test "lookup returns agents for exact match pattern" do
      ...
      agents = AgentRegistry.lookup("com.example.event")
    ...
    test "lookup returns agents for prefix wildcard pattern" do
      ...
      agents = AgentRegistry.lookup("com.example.specific.event")
  ...
  describe "5.2.4 - agent restart recreates subscriptions" do
    ...
    test "restart agent cleans up old registration" do
      ...
      assert {:ok, _info} = AgentRegistry.agent_info(pid)
      ...
      assert {:error, :not_found} = AgentRegistry.agent_info(original_pid)

      # Note: The restarted agent is not automatically re-registered
      # Agents can re-register in their init/1 if needed
      assert {:error, :not_found} = AgentRegistry.agent_info(new_pid)
    end
  end
  ...
  describe "5.2.5 - graceful shutdown stops all agents" do
    ...
    test "stop_all_agents stops all running agents" do
      ...
      {:ok, _pid1} = AgentSupervisor.start_agent(TestAgent10, [], name: :agent1)
      {:ok, _pid2} = AgentSupervisor.start_agent(TestAgent10, [], name: :agent2)
      {:ok, _pid3} = AgentSupervisor.start_agent(TestAgent10, [], name: :agent3)

      assert AgentSupervisor.count() == 3

      # Stop all agents
      :ok = AgentSupervisor.stop_all_agents()
      Process.sleep(100)
    ...
    test "stopping supervisor stops all agents" do
      ...
      assert AgentRegistry.registered?(pid1)
      assert AgentRegistry.registered?(pid2)
      ...
      refute AgentRegistry.registered?(pid1)
      refute AgentRegistry.registered?(pid2)
    end
  end
  ...
  describe "5.2.6 - health monitoring" do
    ...
    test "health_check reports agent status" do
      ...
      health = AgentSupervisor.health_check()
    ...
    test "registry health check detects alive agents" do
      ...
      assert pid in Enum.map(AgentRegistry.list_agents(), fn m -> m.pid end)
    end
  end
  ...
  describe "5.2.7 - list and count agents" do
    ...
    test "count returns correct number of agents" do
      ...
      {:ok, _pid1} = AgentSupervisor.start_agent(TestAgent15, [])
      {:ok, _pid2} = AgentSupervisor.start_agent(TestAgent15, [])
      {:ok, _pid3} = AgentSupervisor.start_agent(TestAgent15, [])
    ...
    test "agent_running? checks agent status" do
      ...
      {:ok, pid} = AgentSupervisor.start_agent(TestAgent16, [], name: :running_agent)

      assert AgentSupervisor.agent_running?(:running_agent) == true
      assert AgentSupervisor.agent_running?(pid) == true
      assert AgentSupervisor.agent_running?(:nonexistent) == false
    end
  end
  ...
  describe "agent_info" do
    ...
    test "agent_info returns agent metadata" do
      ...
      assert {:ok, info} = AgentSupervisor.agent_info(pid)
      assert info.pid == pid
    ...
    test "agent_info returns error for non-existent agent" do
      assert {:error, :not_found} = AgentSupervisor.agent_info(:nonexistent)
      assert {:error, :not_found} = AgentSupervisor.agent_info(self())
    end
  end
  ...
  describe "stop_agent" do
    ...
    test "stop_agent stops a running agent" do
      ...
      assert AgentSupervisor.agent_running?(:stoppable_agent) == true

      :ok = AgentSupervisor.stop_agent(:stoppable_agent)
      Process.sleep(100)

      assert AgentSupervisor.agent_running?(:stoppable_agent) == false
      assert AgentRegistry.registered?(pid) == false
    end
  end
  ...
  describe "registry cleanup" do
    ...
    test "registry cleans up dead agents" do
      ...
      agents = AgentRegistry.lookup("com.cleanup.event")
